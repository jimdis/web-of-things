...
/**
 * Updates data field on model by pushing value aloing with timestamp to data array
 * @param {string} resourceKey id of the resource
 * @param {any} value value to push to data field along with timestamp
 */
const updateDataField = (resourceKey, value) => {
...
module.exports.sendMessage = async message => {
  ...
  const messageSubmitted = await runScript()
  updateDataField('leds', messageSubmitted)
}
...
module.exports.collectData = (interval = 60) => {
  this.sendMessage('Dummy') // to get some initial data to leds
